...
from django.contrib import auth      #(from django.contrib.auth import authenticate, login)此引入的login会与下面的login方法冲突，故import auth 避免）
...
def login(request):
    ...
    if request.method == 'POST':  # 提交数据
        ...
        if login_form.is_valid():  # 是否有效
            user = login_form.cleaned_data['user']
            auth.login(request, user)
            return redirect(request.GET.get('from', reverse('home')))  # 登陆界面为一个新的界面，不能再重定向登陆到当前页面
    else:
...
def register(request):
    if request.method =='POST':
        ...
        if reg_form.is_valid():
            ...
            user.save()
            # 清除session,防止反复用同一个验证码在同一个电脑去注册
