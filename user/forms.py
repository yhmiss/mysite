...
class LoginForm(forms.Form):
    # 变量username_or_email 是form表单接受用户输入的用户名或邮箱
    ...
    password = forms.CharField(label='密码',
                               widget=forms.PasswordInput(attrs={'class':'form-control', 'placeholder':'请输入密码'}))    #密码密文输入
    def clean(self):
...
class RegForm(forms.Form):
    username = forms.CharField(label='用户名',
                               max_length=30,
                               min_length=3,
                               widget=forms.TextInput(attrs={'class': 'form-control', 'placeholder': '请输入3-30用户名'}))
...
class ChangePasswordForm(forms.Form):
    # 变量old_password 是form表单接受用户输入的旧密码
...
class ForgotPasswordForm(forms.Form):
    ...
    def clean_email(self):
        ...
        if not User.objects.filter(email=email).exists():
            raise forms.ValidationError('邮箱未存在')
        return email
